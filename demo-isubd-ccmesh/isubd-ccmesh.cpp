...
enum {
    ...
    BUFFER_GEOMETRY_EDGES,
    BUFFER_GEOMETRY_FACES,
...
void setupSubdKernel(djg_program *djp)
{
    ...
    djgp_push_string(djp, "#define BUFFER_BINDING_GEOMETRY_EDGES %i\n",
                     BUFFER_GEOMETRY_EDGES);
...
bool loadGeometryBuffers()
{
    ...
    LOG("Loading {Mesh-Edge-Buffer}\n");
    if (glIsBuffer(g_gl.buffers[BUFFER_GEOMETRY_EDGES]))
        glDeleteBuffers(1, &g_gl.buffers[BUFFER_GEOMETRY_EDGES]);
    ...
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER,
                     BUFFER_GEOMETRY_EDGES,
                     g_gl.buffers[BUFFER_GEOMETRY_EDGES]);
