...
# if SUBD_IN_PLACE_UPDATE_USE_COMPACTION == 0
    uint threadID = (gl_GlobalInvocationID.x + u_IndirectCommand[6]) % SUBD_BUFFER_CAPACITY;
# else
...
# endif
    ...
    //if (threadID >= atomicCounter(u_SubdBufferCounterEnd)) {
    if (threadID >= u_IndirectCommand[7]) {   //Num triangles is stored in the last reserved field of the draw indiretc structure
        ...
        uvec2 primKey = u_SubdBufferIn[threadID];
#if 1
        if (primKey == uvec2(0, 0)) {
            isVisible = false;

# if SUBD_IN_PLACE_UPDATE_USE_COMPACTION == 0
            //if (threadID > (atomicCounter(u_SubdBufferCounterFront) + atomicCounter(u_SubdBufferCounterEnd))/2  )
...
#   endif
            ...
            updateSubdBuffer(threadID, primID, key, targetLod, parentLod
...
void main()
{
    ...
    //float frontPos = float(atomicCounter(u_SubdBufferCounterFront)) * scaling;
    float frontPos = float(atomicCounter(u_SubdBufferCounterDeleted)) * scaling;
    float endPos = float(atomicCounter(u_SubdBufferCounterEnd)) * scaling;

    if (gl_FragCoord.y < 20.0) {
        if (gl_FragCoord.x > frontPos) {
            if (gl_FragCoord.x < endPos)
                o_FragColor.rgb = vec3(1, 0, 0);
            else
                o_FragColor.rgb = vec3(0, 1, 0);
        }
    }
#endif
